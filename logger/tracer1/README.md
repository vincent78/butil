...
```go
import "github.com/go-dev-frame/sponge/pkg/tracer1"

func initTrace() {
	// exporter := tracer1.NewConsoleExporter() // output to terminal

	// exporter, f, err := tracer1.NewFileExporter("trace.json") // output to file

	// exporter, err := tracer1.NewJaegerExporter("http://localhost:14268/api/traces") // output to jaeger, using collector http
	...
	// tracer1.Init(exporter, resource, 0.5) // collect half
}
```
